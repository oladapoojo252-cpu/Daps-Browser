...
import { Monitor, ShieldAlert, Trash2, X, ChevronRight, FileText } from 'lucide-react-native';
...
export const SettingsView = ({ 
  ...
  const theme = {
    bg: isPrivate ? '#1A1A1A' : '#fff',
    text: isPrivate ? '#fff' : '#000',
    item: isPrivate ? '#222' : '#F9F9F9',
        ...
        <View style={[styles.sheet, { backgroundColor: theme.bg }]} onStartShouldSetResponder={() => true}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={styles.section}>
              ...
              <View style={[styles.row, { backgroundColor: theme.item }]}>
                <View style={styles.rowLeft}>
                  <ShieldAlert size={20} color={isAdBlockActive ? "#A855F7" : theme.text} />
                  <Text style={[styles.rowText, { color: theme.text }]}>Ad-Blocker</Text>
                </View>
                <Switch 
                ...
                downloads.map((item: any, index: number) => (
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.fileTime}>{item.date || item.time}</Text>
                    </View>
            ...
            <View style={styles.footer}>
              ...
              <Text style={[styles.versionText, { color: theme.text, marginTop: 4 }]}>A PRODUCT OF DAPS TECHNOLOGIES</Text>
            </View>
          </ScrollView>
        </View>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  sheet: { 
    ...
    padding: 24, 
    maxHeight: '85%',
    // Extra padding for Android bottom navigation bar
  ...
  emptyText: { textAlign: 'center', color: '#999', marginTop: 10, fontSize: 13, fontStyle: 'italic' },
  footer: { marginTop: 50, paddingBottom: 40, alignItems: 'center', opacity: 0.5 },
